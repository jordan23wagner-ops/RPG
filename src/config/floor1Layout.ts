...
export function createFloor1Layout(): DungeonTileId[][] {
  ...
  const randomFloorTileId = (): DungeonTileId => {
  ...
  const createRandomRoom = (cols: number, rows: number): Room => {
  ...
  const roomIntersects = (room: Room, others: Room[]): boolean =>
    others.some(
      (r) =>
        room.x < r.x + r.w &&
        room.x + room.w > r.x &&
  ...
  const carveRoom = (grid: DungeonTileId[][], room: Room) => {
    const { x, y, w, h } = room;
    for (let iy = y; iy < y + h; iy++) {
      for (let ix = x; ix < x + w; ix++) {
        const isBorder = ix === x || ix === x + w - 1 || iy === y || iy === y + h - 1;
        if (isBorder) {
          grid[iy][ix] = iy === y || iy === y + h - 1 ? 'wall_top' : 'wall_side';
        } else {
  ...
  const carveCorridorCell = (grid: DungeonTileId[][], x: number, y: number) => {
  ...
  const connectAdditionalRooms = (grid: DungeonTileId[][], rooms: Room[]) => {
    ...
    for (let i = 0; i < extraConnections; i++) {
      ...
      connectRooms(grid, roomCenter(rooms[aIndex]), roomCenter(rooms[bIndex]));
    }
  };

  const decorateRooms = (grid: DungeonTileId[][], rooms: Room[]) => {
  ...
  const addTorchesToRoom = (grid: DungeonTileId[][], room: Room) => {
    const { x, y, w, h } = room;
  ...
  const addPropsToRoom = (grid: DungeonTileId[][], room: Room) => {
    const { x, y, w, h } = room;
  ...
  const addBarsAndGrates = (grid: DungeonTileId[][], rooms: Room[]) => {
    if (Math.random() < 0.3) {
  ...
  const cleanInteriorWalls = (grid: DungeonTileId[][], rooms: Room[]) => {
    const rows = grid.length;
    const cols = grid[0].length;
  ...
  const addPillars = (grid: DungeonTileId[][], rooms: Room[]) => {
    for (const room of rooms) {
      const { x, y, w, h } = room;
      if (w < 8 || h < 7) continue;
  ...
  const addHazards = (grid: DungeonTileId[][], rooms: Room[]) => {
  ...
  const isFloorTile = (tile: DungeonTileId): boolean =>
    tile === 'floor_stone_main' ||
    tile === 'floor_stone_alt1' ||
    tile === 'floor_stone_alt2' ||
    tile === 'grate_floor';

  const pickRoomProp = (): DungeonTileId => {
    const choices: DungeonTileId[] = ['crate', 'barrel', 'table', 'wall_column'];
